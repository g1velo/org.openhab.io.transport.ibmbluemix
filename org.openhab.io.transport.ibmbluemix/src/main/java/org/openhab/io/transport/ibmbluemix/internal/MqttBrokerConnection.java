 ...
 */
package org.openhab.io.transport.ibmbluemix.internal;
...
import org.openhab.io.transport.ibmbluemix.MqttMessageConsumer;
import org.openhab.io.transport.ibmbluemix.MqttMessageProducer;
import org.openhab.io.transport.ibmbluemix.MqttSenderChannel;
import org.openhab.io.transport.ibmbluemix.MqttWillAndTestament;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * An MQTTBrokerConnection represents a single client connection to a IBM Bluemix MQTT
 * broker. The connection is configured by the IBM Bluemix MQTTService with properties from
 * the openhab.cfg file.
 *
 * When a connection to an IBM Bluemix MQTT broker is lost, it will try to reconnect every
 * 60 seconds.
 *
...
public class MqttBrokerConnection implements MqttCallback {
    ...
    public synchronized void start() throws Exception {
        ...
        if (StringUtils.isEmpty(url)) {
            logger.debug("No url defined for IBM Bluemix MQTT broker connection '{}'. Not starting.", name);
            return;
        }

        logger.info("Starting IBM Bluemix MQTT broker connection '{}'", name);
        openConnection();
    ...
    /**
     * Get the url for the IBM Bluemix MQTT broker. Valid URL's are in the format:
     * tcp://localhost:1883 or ssl://localhost:8883
     *
     * @return url for the IBM Bluemix MQTT broker.
     */
    ...
    /**
     * Set the url for the IBM Bluemix MQTT broker. Valid URL's are in the format:
     * tcp://localhost:1883 or ssl://localhost:8883
     *
     * @param url
     *            url string for the IBM Bluemix MQTT broker.
     */
    ...
    /**
     * @return optional user name for the IBM Bluemix MQTT connection.
     */
    ...
    /**
     * Set the optional user name to use when connecting to the IBM Bluemix MQTT broker.
     *
     * @param user
    ...
    /**
     * Set the optional password to use when connecting to the IBM Bluemix MQTT broker.
     *
     * @param password
     */
    ...
    /**
     * Open an IBM Bluemix MQTT client connection.
     *
     * @throws Exception
     */
    ...
    /**
     * Close the IBM Bluemix MQTT connection.
     */
    ...
    public synchronized void connectionLost(Throwable t) {
        ...
        logger.error("IBM Bluemix MQTT connection to broker was lost", t);

        if (t instanceof MqttException) {
            MqttException e = (MqttException) t;
            logger.error("IBM Bluemix MQTT connection to '{}' was lost: {} : ReasonCode {} : Cause : {}",
                    new Object[] { name, e.getMessage(), e.getReasonCode(),
                            (e.getCause() == null ? "Unknown" : e.getCause().getMessage()) });
        } else {
            logger.error("IBM Bluemix MQTT connection to '{}' was lost: {}", name, t.getMessage());
        }
